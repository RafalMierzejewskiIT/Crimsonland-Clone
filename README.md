...
`fire()` function checks if `SelectedWeapon.rate_of_fire_timer <= 0 and SelectedWeapon.ammo_current > 0` Player will successfully fire, if `SelectedWeapon.ammo_current <= 0` the weapon will be reloaded to `ammo_max`. <br/>
...
If the weapon didn't finish reloading, the timer is reset.
...
Every bullet faces the direction of the cursor using previously described trigonometry. <br/>
...
This project further increased my already strong stance on the importance of planning before making, after you lay down some infrastructure, changing it is thrice as hard as just making it right in the first place. </br>
